...
async def get_summary(user_id: int) -> Optional[dict[str, Any]]:
...
async def get_alerts(user_id: int) -> Optional[list[Any]]:
    """Read recent health alerts from cache."""
    key = make_key(RedisKeys.HEALTH_ALERTS, user_id=user_id)
    return await cache_get(key)


async def set_alerts(user_id: int, alerts: list[dict[str, Any]]) -> None:
