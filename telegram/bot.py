    ...
    async def on_grant(event: events.NewMessage.Event) -> None:
        ...
        try:
            ...
            if success:
                ...
                try:
                    caption = (
                        f"<tg-emoji emoji-id='5206607081334906820'>✅</tg-emoji> <b>Lucid Ads Premium - {plan.capitalize()}</b>\n\n"
                        f"Your {plan.lower()} plan has been activated. It will end on {ends_at.strftime('%B %d, %Y')}."
                    )
